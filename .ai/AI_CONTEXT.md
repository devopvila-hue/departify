...
`packages/executive-orchestrator` is the official composition boundary that wires Executive Director into the runtime flow. It defines `OrchestratorIntent` types (`health_check`, `organization_summary`, `generate_identifier`, `discovery_analyze`), the `ExecutiveDecisionMapper` (the only component authorised to translate `ExecutiveDecision` into `AgentToolAction`), and the `ExecutiveOrchestrator` facade that drives the pipeline `OrchestratorIntent → ExecutiveDirector → DecisionMapper → AgentToolBridge → Tool Runtime → Core Tool Catalog`. Sprint 30 adds `discovery_analyze`, which dispatches to the `discovery.analyze` Core Tool through the same pipeline, with dynamic Tool args carried on the intent's `toolArgs` slot. Sprint 31 adds `ExecutiveDiscoveryWorkflow`, the first official Executive workflow: it composes `BusinessDiscoveryService.initiateDiscovery` (Sprint 28) with `orchestrateDiscoveryAnalyze` (Sprint 30) into a typed `CompanyDiscoveryReport` flow. It preserves the full correlation chain (intentId → decisionId → actionId) inside the `OrchestrationResult` envelope. The orchestrator depends only on public contracts of Executive Director, AgentToolBridge, Agent Runtime, Tool Runtime, Tool Catalog and Business Discovery; it never modifies them. No IA, no LLM Router, no HTTP, no SDKs, no Fastify, no Supabase, no Docker, no plugins, no MCP.

`packages/departments` is the **external product unit** (Sprint 24). A Department composes references to existing components: Executive Director (director), Agent Runtime (digital employees), Tool Runtime + Core Tool Catalog (tools), Knowledge Engine (knowledge collections), Memory Engine (memory sessions), Workflows (workflow ids), Business Discovery (optional discovery id — Sprint 34, the Empresa Digital references the discovery of its business) and a placeholder for connected applications. It owns identity, configuration, lifecycle states (`draft`, `active`, `paused`, `archived`), metrics and an event taxonomy (`department.created`, `department.activated`, `department.employee_added`, `department.tool_associated`, `department.discovery_associated`, etc.). The Department never duplicates logic — it only carries references. Sprint 24 ships the canonical `Comercial` demo department with four Digital Employees (`agent_sales_director`, `agent_lead_qualifier`, `agent_outreach_specialist`, `agent_proposal_writer`) and pre-associated Tools, Knowledge Collections and Memory Sessions. Sprint 51 adds the `Marketing` template (`tpl_marketing`, Customer Zero) with its Director (`agent_marketing_director`) and three Digital Employees. The package must not import provider SDKs, read environment variables, perform HTTP calls, depend on the LLM Router, or contain IA.
...
`packages/workflows` is the first Department collaboration layer. It owns `WorkflowDefinition`, `WorkflowStep`, `WorkflowExecution`, `WorkflowResult`, `WorkflowBuilder`, the canonical `Lead Qualification Workflow` (`wf_lead_qualification`), the `Business Briefing Workflow` (`wf_business_briefing`, Sprint 40 — the first useful work a freshly provisioned Department performs: the Sales Director reads the organization's Business Discovery report through `discovery.get`), the `Business Readiness Workflow` (`wf_business_readiness`, Sprint 41 — the first automatic decision: the Sales Director runs `discovery.readiness` for a deterministic go/no-go) and the `Department Plan Workflow` (`wf_department_plan`, Sprint 42 — the first automatic planning: the Sales Director runs `discovery.plan` to order the discovery questions by priority) and the `Department Delegation Workflow` (`wf_department_delegation`, Sprint 43 — the first automatic delegation: the Sales Director runs `discovery.delegate` to assign the plan items to the competent Digital Employees) and the `First Work Workflow` (`wf_first_work`, Sprint 44 — the first work executed by a Digital Employee: the delegated employee runs `discovery.get` to know the business it is about to serve) and the `First Result Workflow` (`wf_first_result`, Sprint 45 — the first useful result produced by a Digital Employee: the delegated employee runs `discovery.summary` for the executive summary of the business). Sprint 46 adds `WorkResultRepository` (provider-neutral port) with the default `InMemoryWorkResultRepository` so completed work results are recoverable by the Empresa Digital (Departify sells finished work; the finished work is now persisted). Sprint 47 adds the `Department Onboarding Workflow` (`wf_department_onboarding`) — the Vending Machine step: a single trigger runs the whole Department pipeline (briefing → readiness → plan → delegation → first work → first result) and delivers the first value. Sprint 52 parameterises the onboarding Director (`buildDepartmentOnboardingWorkflow(organizationId, directorAgentId, employeeAgentId)`) so the contracted Department — e.g. the Marketing Customer Zero — learns the business with its own Director. Workflows are pure composition over `AgentToolBridge` and `Tool Runtime`; the execution engine dispatches each step through the bridge (one Digital Employee per step) and threads the previous step's output into the next step's metadata. Sprint 26 ships the Comercial workflow with three steps (Qualify Lead, Prepare Contact, Generate Proposal) executed by `agent_lead_qualifier`, `agent_outreach_specialist` and `agent_proposal_writer`. The Department aggregate tracks the workflow ids attached to it; no logic duplication. No IA, no LLM Router, no HTTP, no SDKs, no Fastify, no Supabase, no Docker.

`packages/business-events` is the official event-driven composition layer. It owns `BusinessEvent` (with six typed event types: `payment.confirmed`, `lead.created`, `organization.created`, `organization.provisioned`, `organization.discovery_requested`, `organization.discovered`), the `BusinessEventCatalog` (the only authorised source for event → handler mapping) and the `BusinessEventService` that publishes events through the catalog. The service never executes business logic directly; every handler delegates to an existing runtime (`WorkflowExecution`, `BusinessProvisioningService`, `ExecutiveOrchestrator`, `ExecutiveDiscoveryWorkflow`) or a host-supplied reaction (`DiscoveryCompletionHandler`, `OrganizationCreator`). Sprint 49 adds `payment.confirmed` — the Vending Machine entry point: the external Stripe event (real or simulated with the same shape) turns a paid customer into an organization via the `OrganizationCreator` port. Sprint 50 makes a single `payment.confirmed` put the whole Empresa Digital in motion: after creating the organization it runs the shared provisioning pipeline (activation → discovery → Department onboarding → first value, Sprints 38/48) with no manual trigger. The service never executes business logic directly; every handler delegates to an existing runtime (`WorkflowExecution`, `BusinessProvisioningService`, `ExecutiveOrchestrator`, `ExecutiveDiscoveryWorkflow`) or a host-supplied reaction (`DiscoveryCompletionHandler`, `OrganizationCreator`). Results are idempotent by eventId and carry the full correlation chain (eventId, workflowId, executionId, provisioningId, timestamps, errors). No reflection, no dynamic discovery, no IA, no LLM Router, no HTTP, no SDKs, no Fastify, no Supabase, no Docker.

`packages/tool-catalog` is the official catalog of core Tools. It owns eleven Tool definitions: `system.uuid` (uuid + version), `organization.get` (reads the active organization through `packages/organization-domain`), `memory.search` (typed `MemoryRetrievalPort` from `packages/memory-engine`), `knowledge.search` (typed `KnowledgeRetrievalPort` from `packages/knowledge-engine`), `system.health` (typed runtime + tool runtime + router + provider registry snapshot), `discovery.analyze` (deterministic Gap Analysis + Question Generation over a supplied `CompanyDNA` and optional `FounderBrain`, delegating to `packages/business-discovery` — Sprint 29), `discovery.get` (returns the most recent `CompanyDiscoveryReport` of an organization from a `DiscoveryReportRepository` — Sprint 37), `discovery.readiness` (deterministic go/no-go decision on whether the Empresa Digital is ready to operate, derived from blocking gaps of the most recent report — Sprint 41), `discovery.plan` (deterministic first work plan of the Department: the discovery questions of the most recent report ordered by priority — Sprint 42), `discovery.delegate` (deterministic delegation of the plan items to the competent Digital Employees of the Comercial Department via a category → agent mapping — Sprint 43) and `discovery.summary` (deterministic executive summary of the business: confidence, gap counts, question count — Sprint 45, the first useful result a Digital Employee produces). It exposes `registerAllCoreTools(registry, context)` as the single composition point: hosts must call it exactly once. The catalog depends only on contracts already exposed by `packages/organization-domain`, `packages/memory-engine`, `packages/knowledge-engine`, `packages/llm-router` and `packages/business-discovery` (types only). It must not create new runtimes, bridges, packages, IA, embeddings, vector search, HTTP clients, Fastify, Supabase, Docker, plugins, MCP, or external SDKs.
