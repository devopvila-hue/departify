...
`packages/workflows` is the first Department collaboration layer. It owns `WorkflowDefinition`, `WorkflowStep`, `WorkflowExecution`, `WorkflowResult`, `WorkflowBuilder`, the canonical `Lead Qualification Workflow` (`wf_lead_qualification`), the `Business Briefing Workflow` (`wf_business_briefing`, Sprint 40 — the first useful work a freshly provisioned Department performs: the Sales Director reads the organization's Business Discovery report through `discovery.get`), the `Business Readiness Workflow` (`wf_business_readiness`, Sprint 41 — the first automatic decision: the Sales Director runs `discovery.readiness` for a deterministic go/no-go) and the `Department Plan Workflow` (`wf_department_plan`, Sprint 42 — the first automatic planning: the Sales Director runs `discovery.plan` to order the discovery questions by priority) and the `Department Delegation Workflow` (`wf_department_delegation`, Sprint 43 — the first automatic delegation: the Sales Director runs `discovery.delegate` to assign the plan items to the competent Digital Employees) and the `First Work Workflow` (`wf_first_work`, Sprint 44 — the first work executed by a Digital Employee: the delegated employee runs `discovery.get` to know the business it is about to serve). Workflows are pure composition over `AgentToolBridge` and `Tool Runtime`; the execution engine dispatches each step through the bridge (one Digital Employee per step) and threads the previous step's output into the next step's metadata. Sprint 26 ships the Comercial workflow with three steps (Qualify Lead, Prepare Contact, Generate Proposal) executed by `agent_lead_qualifier`, `agent_outreach_specialist` and `agent_proposal_writer`. The Department aggregate tracks the workflow ids attached to it; no logic duplication. No IA, no LLM Router, no HTTP, no SDKs, no Fastify, no Supabase, no Docker.

`packages/business-events` is the official event-driven composition layer. It owns `BusinessEvent` (with five typed event types: `lead.created`, `organization.created`, `organization.provisioned`, `organization.discovery_requested`, `organization.discovered`), the `BusinessEventCatalog` (the only authorised source for event → handler mapping) and the `BusinessEventService` that publishes events through the catalog. The service never executes business logic directly; every handler delegates to an existing runtime (`WorkflowExecution`, `BusinessProvisioningService`, `ExecutiveOrchestrator`, `ExecutiveDiscoveryWorkflow`) or a host-supplied reaction (`DiscoveryCompletionHandler`). Sprint 32 wires `organization.discovery_requested` to the `ExecutiveDiscoveryWorkflow` (Sprint 31); Sprint 33 adds the `organization.discovered` fact event; Sprint 35 adds a default handler that, when no host completion handler is supplied, associates the completed discovery to the organization's non-archived Departments through the `DepartmentService` (Sprint 34 capability); Sprint 38 makes the `organization.provisioned` handler run the `ExecutiveDiscoveryWorkflow` automatically after a successful activation when a discovery workflow is wired — completing the end-to-end flow for creating an Empresa Digital. Sprint 39 makes the `lead.created` handler turn a qualified lead into an organization automatically when an `OrganizationCreator` port is wired, closing the first step of the end-to-end flow. Results are idempotent by eventId and carry the full correlation chain (eventId, workflowId, executionId, provisioningId, timestamps, errors). No reflection, no dynamic discovery, no IA, no LLM Router, no HTTP, no SDKs, no Fastify, no Supabase, no Docker. Sprint 27 ships the first three default handlers; Sprint 32 adds the discovery request handler; Sprint 33 adds the discovery completion handler; Sprint 35 wires its default Department association fallback; Sprint 38 adds the automatic discovery step on provisioning; Sprint 39 adds the qualified-lead → organization step.

`packages/tool-catalog` is the official catalog of core Tools. It owns ten Tool definitions: `system.uuid` (uuid + version), `organization.get` (reads the active organization through `packages/organization-domain`), `memory.search` (typed `MemoryRetrievalPort` from `packages/memory-engine`), `knowledge.search` (typed `KnowledgeRetrievalPort` from `packages/knowledge-engine`), `system.health` (typed runtime + tool runtime + router + provider registry snapshot), `discovery.analyze` (deterministic Gap Analysis + Question Generation over a supplied `CompanyDNA` and optional `FounderBrain`, delegating to `packages/business-discovery` — Sprint 29), `discovery.get` (returns the most recent `CompanyDiscoveryReport` of an organization from a `DiscoveryReportRepository` — Sprint 37), `discovery.readiness` (deterministic go/no-go decision on whether the Empresa Digital is ready to operate, derived from blocking gaps of the most recent report — Sprint 41), `discovery.plan` (deterministic first work plan of the Department: the discovery questions of the most recent report ordered by priority — Sprint 42) and `discovery.delegate` (deterministic delegation of the plan items to the competent Digital Employees of the Comercial Department via a category → agent mapping — Sprint 43). It exposes `registerAllCoreTools(registry, context)` as the single composition point: hosts must call it exactly once. The catalog depends only on contracts already exposed by `packages/organization-domain`, `packages/memory-engine`, `packages/knowledge-engine`, `packages/llm-router` and `packages/business-discovery` (types only). It must not create new runtimes, bridges, packages, IA, embeddings, vector search, HTTP clients, Fastify, Supabase, Docker, plugins, MCP, or external SDKs.
