    ...
    const { error } = await this.admin.from("organization_tool_states").upsert(
      {
        ...
        config_source: state.configSource ?? null,
        connection_provider: state.provider ?? null,
        provider_account_ref: state.providerAccountRef ?? null,
        granted_capabilities: state.grantedCapabilities ? [...state.grantedCapabilities] : null,
        granted_scopes: state.grantedScopes ? [...state.grantedScopes] : null,
        verified_at: state.verifiedAt ?? null,
        last_validated_at: state.lastValidatedAt ?? null,
